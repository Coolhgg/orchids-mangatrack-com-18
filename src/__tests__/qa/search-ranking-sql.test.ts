...
describe('Search Ranking SQL Query', () => {
  ...
  describe('seriesSearch integration', () => {
    ...
    it('should filter by safe browsing mode', () => {
      ...
      // Questionable mode: safe, suggestive, questionable
      const questionableFilter = (rating: string | null) =>
        rating === null || ['safe', 'suggestive', 'questionable'].includes(rating!);
      
      // NSFW mode: all
      const nsfwFilter = () => true;

      expect(contentRatings.filter(sfwFilter)).toEqual(['safe', 'suggestive']);
      expect(contentRatings.filter(questionableFilter)).toEqual(['safe', 'suggestive']);
      expect(contentRatings.filter(nsfwFilter)).toEqual(contentRatings);
    });
  });
