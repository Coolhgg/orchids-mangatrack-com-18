...
export async function getCachedSearchResult(
  ...
  try {
    ...
    if (cached) {
      await redis.hincrby(`${SEARCH_STATS_PREFIX}global`, 'hits', 1).catch(() => {});
      try {
    ...
    await redis.hincrby(`${SEARCH_STATS_PREFIX}global`, 'misses', 1).catch(() => {});
    return null;
  } catch (err) {
...
export async function waitForPendingSearch(
  ...
  while (Date.now() - startTime < maxPendingWaitMs) {
    ...
    if (cached) {
      await redis.hincrby(`${SEARCH_STATS_PREFIX}global`, 'dedup_saves', 1).catch(() => {});
      return cached;
    }
...
export async function checkExternalSearchDedup(
  ...
  try {
    ...
    if (existingJobId) {
      await redis.hincrby(`${SEARCH_STATS_PREFIX}global`, 'external_dedup_saves', 1).catch(() => {});
